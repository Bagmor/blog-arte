...
$sql_posts = "
    SELECT 
        analises.id, analises.titulo_analise, 
        obras.titulo_obra, obras.nome_artista, obras.url_imagem,
        movimentos_arte.nome_movimento,
        movimentos_arte.slug AS movimento_slug
    FROM analises
    JOIN obras ON analises.obra_id = obras.id
    JOIN movimentos_arte ON obras.movimento_id = movimentos_arte.id
...
<head>
    ...
    <title>Fragmentos de Tela - Análises de Arte</title>
...
<body>
    ...
    <div class="container">
        <header class="main-header">
            <div class="logo">
                <h1><a href="index.php">Fragmentos de Tela</a></h1>
                <p>Análises e teorias sobre obras de arte.</p>
            </div>
            <nav class="main-nav">
                <ul>
                    ...
                    <?php foreach ($movimentos as $movimento): ?>
                        <li>
                            <a href="#" class="filter-btn" data-filter="<?= htmlspecialchars($movimento['slug']) ?>">
                                <?= htmlspecialchars($movimento['nome_movimento']) ?>
                            </a>
                        </li>
                    <?php endforeach; ?>
                </ul>
        ...
        <section class="creator-section">
            ...
            <form id="create-card-form" class="hidden">
                ...
                <div class="form-group"><label for="movement">Movimento Artístico</label>
                    <select id="movement">
                         <?php foreach ($movimentos as $movimento): ?>
            ...
            <?php foreach ($posts as $post): ?>
                <article class="post-card" data-category="<?= htmlspecialchars($post['movimento_slug']) ?>" data-id="<?= htmlspecialchars($post['id']) ?>">
                    <div class="card-image">
                        <a href="#"><img src="<?= htmlspecialchars($post['url_imagem']) ?>" alt="<?= htmlspecialchars($post['titulo_obra']) ?>"></a>
                    </div>
